...
class CardAccessReader:
    ...
    def read(self) -> bytes:
        ...
        try:
            self._iso7816.select_elementary_file(EF_CARD_ACCESS_FID)
        except ISO7816Exception as exc:
    ...
    def _read_all(self) -> bytes:
        ...
        try:
            header = self._iso7816.read_binary(0, 4)
        except ISO7816Exception as exc:
            raise CardAccessReadError(
                f"Could not read EF.CardAccess header (SW={(exc.sw1 or 0):02X}{(exc.sw2 or 0):02X}: {exc.data}).",
                sw1=exc.sw1,
                sw2=exc.sw2,
            ) from exc

        if len(header) < 2:
            raise CardAccessReadError("EF.CardAccess is too short to contain a SecurityInfos blob.")
        ...
        try:
            body_len, len_size = asn1_length(header[1:])
        ...
        while offset < total:
            ...
            try:
                chunk = self._iso7816.read_binary(offset, to_read)
            except ISO7816Exception as exc:
                raise CardAccessReadError(
