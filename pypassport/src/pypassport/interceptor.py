...
* **Interactive interception** (``enabled = True``): a synchronous
  ...
  thread until the user clicks Forward or Drop. Optional ``breakpoints`` scope
  this mode to matching APDUs so a long read can run without stopping on every
  command.
...
class Interceptor(Singleton):
    ...
    def __init__(self):
        ...
        # Interactive interception can be scoped to matching commands. When
        # empty, every APDU is held as before; when populated, only APDUs
        # matching at least one enabled breakpoint reach the callback.
        self.breakpoints: list[Rule] = []
    ...
    def intercept(self, apdu: "APDUCommand") -> Optional["APDUCommand"]:
        ...
        * Otherwise the match-&-replace rules are applied and the resulting
        ...
        if self.enabled and self.callback is not None:
            if self.breakpoints and not any(rule.enabled and rule.matches(apdu) for rule in self.breakpoints):
                return apdu
    ...
    def add_breakpoint(self, rule: Rule) -> None:
