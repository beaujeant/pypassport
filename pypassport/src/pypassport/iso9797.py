...
def mac(key: bytes, msg: bytes) -> bytes:
    ...
    if len(msg) % 8:
        raise ValueError("mac() requires a message padded to an 8-byte boundary")

    iv = b"\x00" * 8
    tdesa = DES.new(key[0:8], DES.MODE_CBC, iv)

    cb = tdesa.encrypt(msg[:8])
    for offset in range(8, len(msg), 8):
        cb = tdesa.encrypt(msg[offset : offset + 8])
