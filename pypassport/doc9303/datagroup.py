...
class DataGroupFile(object):
    ...
    def _setHeader(self, value):
        self.__header = value
        if value != "":
            self.__tag = converter.toTAG(hexfunctions.binToHexRep(value[0]))
...
class DataGroupReader():
    ...
    def _readHeader(self, dg):
        ...
        (self._bodySize, self.offset) = asn1.asn1Length(header[1:])
        self.offset += 1

        logging.debug("Value 1: {} - Value 2: {}".format(int(converter.toTAG(dg), 16), header[0]))
        if(int(converter.toTAG(dg), 16) != header[0]):
            raise Exception("Wrong AID: " + hexfunctions.binToHexRep(header[0]) + " instead of " + str(self.file.tag))
...
class SFIDataGroupReader(DataGroupReader):
    ...
    def _selectFile(self, tag):
        ...
        SFI = (hexfunctions.hexRepToHex(converter.toSEF(tag)) ^ 0x80) * 256
        self._offset = SFI
