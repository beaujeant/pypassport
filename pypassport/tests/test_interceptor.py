...
def test_interactive_breakpoint_only_holds_matching_apdus():
    interceptor = Interceptor()
    interceptor.enabled = True
    held = []

    def callback(apdu):
        held.append(apdu.ins)
        return apdu

    interceptor.callback = callback
    interceptor.add_breakpoint(Rule(match={"ins": "A4"}))

    passthrough = interceptor.intercept(_cmd(ins="B0"))
    matched = interceptor.intercept(_cmd(ins="A4"))

    assert passthrough.ins == "B0"
    assert matched.ins == "A4"
    assert held == ["A4"]


def test_disabled_breakpoint_does_not_hold():
    interceptor = Interceptor()
    interceptor.enabled = True
    interceptor.callback = lambda apdu: None
    interceptor.add_breakpoint(Rule(match={"ins": "A4"}, enabled=False))

    out = interceptor.intercept(_cmd(ins="A4"))
...
def test_transmit_applies_edit_before_send():
    Interceptor().enabled = True
    Interceptor().callback = lambda apdu: APDUCommand(apdu.cla, apdu.ins, "08", "04", data=apdu.data)
    reader = FakeReader()
    iso = ISO7816(reader)
    iso.transmit(_cmd(ins="A4", p1="02", p2="0C", data="3F00"))
