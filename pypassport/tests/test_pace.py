...
# ---------------------------------------------------------------------------
# 7. Full perform_pace integration — mock transport
# ---------------------------------------------------------------------------
...
class TestPerformPACE:
    """
    Full perform_pace integration test with a deterministic mock transport.
    ...
    * The mock transmit() delivers those responses to perform_pace().
    * After perform_pace(), we verify that iso7816.ciphering is an
    ...
    def _make_iso7816_mock(self, scenario: dict):
        ...
        iso.general_authenticate.side_effect = fake_general_authenticate
        iso.transmit.side_effect = fake_transmit
        iso.mse_set_at.return_value = None
        return iso
    ...
    def test_performPACE_installs_aes_sm(self):
        ...
        pace.perform_pace(_OID_128, b"\x01")
...
class TestAesSecureMessaging:
    ...
    def test_unprotect_wrapped_error_advances_ssc(self):
        """An SM-wrapped error response must still be MAC-verified and advance
        the SSC, keeping the channel in lockstep with the chip. Regression for
        a skipped response-side SSC step that wedged every later command after
        an access-denied (e.g. EAC-protected) data group."""
        sm = self._make_sm()
        cmd = APDUCommand("00", "B0", "00", "00", le="08")
        sm.protect(cmd)  # command-side step: ssc 0 -> 1
        assert int.from_bytes(sm._ssc, "big") == 1

        # The chip wraps the 6982 status in DO'99' and MACs it with its own
        # response-side SSC value (2).
        ssc_chip = (2).to_bytes(16, "big")
        do99 = b"\x99\x02\x69\x82"
        K = _iso_pad(ssc_chip + do99)
        c = CMAC.new(self._K_MAC, ciphermod=AES)
        c.update(K)
        do8e = b"\x8e\x08" + c.digest()[:8]
        raw = do99 + do8e
        # Outer status word echoes the error; the real status lives in DO'99'.
        rapdu = APDUResponse(raw, 0x69, 0x82)

        result = sm.unprotect(rapdu)
        assert (result.sw1, result.sw2) == (0x69, 0x82)
        # Response-side step happened: counter now in lockstep with the chip.
        assert int.from_bytes(sm._ssc, "big") == 2

    def test_unprotect_bare_error_leaves_ssc(self):
        """A bare error (no Secure Messaging data objects) is returned
        unchanged and must not touch the SSC."""
        sm = self._make_sm()
        cmd = APDUCommand("00", "B0", "00", "00", le="08")
        sm.protect(cmd)  # ssc 0 -> 1
        rapdu = APDUResponse(b"", 0x6A, 0x82)

        result = sm.unprotect(rapdu)
        assert (result.sw1, result.sw2) == (0x6A, 0x82)
        assert int.from_bytes(sm._ssc, "big") == 1
