...
def test_dg15_tag():
    assert converter.to_tag("DG15") == "6F"


def test_dg15_fid():
    """EF.DG15 file identifier must be 010F per ICAO 9303 Part 10."""
    assert converter.to_fid("DG15") == "010F"


def test_dg15_fid_from_tag():
    """Same FID whether we look up by DG name or by tag."""
    assert converter.to_fid("6F") == "010F"


def test_dg16_fid():
    """EF.DG16 file identifier must be 0110 per ICAO 9303 Part 10."""
    assert converter.to_fid("DG16") == "0110"


def test_dg14_fid_unaffected():
    """DG14 FID should remain 010E (sequential with DG1-DG14)."""
    assert converter.to_fid("DG14") == "010E"


# ---------------------------------------------------------------------------
# read_elementary_file selects the correct FID for DG15
# ---------------------------------------------------------------------------
...
_FAKE_SPKI = bytes(
    [
        ...
        0x03,
        0x00,  # BIT STRING (empty placeholder)
    ]
)


def test_readElementaryFile_selects_dg15_fid_010f():
    """read_elementary_file must SELECT FID 010F when reading tag 6F / DG15."""
    ...
    read_elementary_file("DG15", iso)

    assert iso.selected_fid == "010F", (
        f"Expected FID 010F for DG15 but got {iso.selected_fid!r}. "
...
def test_readElementaryFile_selects_dg15_by_tag():
    ...
    read_elementary_file("6F", iso)

    assert iso.selected_fid == "010F"


def test_read_elementary_file_keeps_unexpected_outer_tag_visible():
    dg_bytes = bytearray(_minimal_dg15_bytes(_FAKE_SPKI))
    dg_bytes[0] = 0x70
    iso = FakeISO7816(bytes(dg_bytes))

    dg = read_elementary_file("DG15", iso)

    assert dg.tag == "6F"
    assert dg["actual_outer_tag"] == "70"
    assert dg["expected_outer_tag"] == "6F"
    assert any(error["context"] == "outer_tag" for error in dg["parse_errors"])
...
def test_dg15_uses_sm_session(monkeypatch):
    """
    read_elementary_file must go through iso7816.select_elementary_file and
    iso7816.read_binary, which use the active ciphering object.  We verify
    ...
    # read_binary returns the 4-byte header first, then the body.
    iso.read_binary.side_effect = [
    ...
    iso.select_elementary_file.assert_called_once_with("010F")
    assert iso.read_binary.call_count >= 1
...
def test_dg15_chip_error_message_includes_sw(caplog):
    """
    When SM is active and the chip returns a non-success SW for DG15, the
    logged message must include the SW values, not the ambiguous 'Secure
    messaging not supported' string by itself. The read then returns None.
    """
    import logging
    ...
    with (
        patch("pypassport.epassport.read_elementary_file", side_effect=sm_err),
        ...
        # converter.to_tag("DG15") returns "6F"; "6F" not in ep (empty dict).
        result = ep["DG15"]
...
def _build_valid_sm_response(command_ssc: bytes, plain_data: bytes, inner_sw: bytes = b"\x90\x00") -> "APDUResponse":
    """
