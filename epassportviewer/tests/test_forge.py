...
from epassportviewer.apdu_format import (
    assemble_apdu,
    describe_apdu,
    describe_apdu_fields,
    parse_apdu,
    parse_apdu_lenient,
)
...
def test_parse_then_assemble_round_trips(label, raw):
    f = parse_apdu(raw)
    assert assemble_apdu(f["cla"], f["ins"], f["p1"], f["p2"], f["lc"], f["data"], f["le"]) == raw.upper()
...
def test_describe_select_and_read_binary():
    assert describe_apdu("00A4040C07A0000002471001") == "SELECT FILE (eMRTD AID)"
    assert describe_apdu_fields("00", "B0", "00", "10", "", "", "04") == "READ BINARY (offset 16)"


def test_lenient_parse_preserves_length_inconsistent_probe():
    fields = parse_apdu_lenient("00A4020C053F00")
    assert fields == {
        "cla": "00",
        "ins": "A4",
        "p1": "02",
    ...
    assert assemble_apdu(**fields) == "00A4020C053F00"
