...
class _ForgeRequest:
    ...
    def __init__(self, cla="", ins="", p1="", p2="", lc="", data="", le="", raw_mode=False, label="", raw=""):
        self.cla = cla
        self.ins = ins
        self.p1 = p1
        self.p2 = p2
        ...
        # The APDUTransaction recorded for the last send. None until the tab has
        # been sent at least once; used to tell a pristine tab from a used one.
        self.last_tx = None
...
class ForgePane:
    def __init__(self, main):
        ...
        # Themed colours for the numbered request buttons (plain tk.Buttons, so
        # they don't follow the ttk styling). The active tab uses the accent
        # colour so the current request is obvious at a glance.
        self._tab_bg = theme.SURFACE_ALT
        self._tab_active_bg = theme.ACCENT
        ...
        ttk.Button(tabbar, text="Duplicate", command=self._duplicate_request).pack(side="right", padx=2)
        ttk.Button(tabbar, text="Rename", command=self._rename_request).pack(side="right", padx=2)
        ...
        ttk.Label(row0, text="Name:").pack(side="left", padx=(12, 2))
        self._label = tk.StringVar()
        self._label.trace_add("write", self._on_label_changed)
        ttk.Entry(row0, textvariable=self._label, width=22).pack(side="left")
        ...
        ttk.Checkbutton(
            row0,
            text="Raw (paste full APDU hex)",
            variable=self._raw_mode,
            command=self._toggle_raw,
        ).pack(side="right", padx=8)

        # Fielded view: CLA / INS / P1 / P2 / LC / LE on one row, DATA below.
        self._fields_row = ttk.Frame(req_frame)
        self._cla = tk.StringVar()
        self._ins = tk.StringVar()
        self._p1 = tk.StringVar()
        self._p2 = tk.StringVar()
        self._lc = tk.StringVar()
        self._le = tk.StringVar()
        for label, width, var in (
            ("CLA", 3, self._cla),
            ("INS", 3, self._ins),
            ("P1", 3, self._p1),
            ("P2", 3, self._p2),
        ...
        ttk.Entry(self._raw_row, textvariable=self._raw, width=80).pack(side="left", fill="x", expand=True, padx=(0, 8))
        ...
        self._sm_combo = ttk.Combobox(
            self._sm_row,
            textvariable=self._sm_var,
            state="readonly",
            width=14,
            values=["None"],
            postcommand=self._refresh_sm_options,
        )
        self._sm_combo.pack(side="left")
        self._sm_combo.bind("<<ComboboxSelected>>", self._on_sm_choice)

        ttk.Button(self._sm_row, text="Send APDU", command=self._send, style="Accent.TButton").pack(
            side="right", padx=8
        )
        ttk.Button(self._sm_row, text="Fuzz request", command=self._send_to_fuzzer).pack(side="right", padx=3)
        ttk.Button(self._sm_row, text="Copy response", command=self._copy_response).pack(side="right", padx=3)
        ttk.Button(self._sm_row, text="Copy request", command=self._copy_request).pack(side="right", padx=3)

        self._request_summary = tk.StringVar(value="No request loaded.")
        ttk.Label(req_frame, textvariable=self._request_summary, style="Muted.TLabel").pack(
            anchor="w", padx=8, pady=(2, 0)
        )
        for var in (self._cla, self._ins, self._p1, self._p2, self._lc, self._data, self._le, self._raw):
            var.trace_add("write", self._on_request_changed)
        ...
        ttk.Button(sm_row, text="Redo BAC", command=partial(self._reestablish, MODE_BAC)).pack(side="left", padx=3)
        ttk.Button(sm_row, text="Redo PACE", command=partial(self._reestablish, MODE_PACE)).pack(side="left", padx=3)
        self._sm_status = tk.StringVar()
        ttk.Label(sm_row, textvariable=self._sm_status, style="Muted.TLabel").pack(side="left", padx=(12, 4))
        ...
        ttk.Label(resp_frame, textvariable=self._resp_status, style="Muted.TLabel").pack(
    ...
    def _render_tabstrip(self):
        ...
        for i in range(len(self._requests)):
            ...
            fg = theme.TEXT_ON_ACCENT if active else theme.TEXT
            label = self._request_tab_label(i)
            tk.Button(
                self._tabstrip,
                text=label,
                relief="flat",
    ...
    def _commit_active(self):
        ...
        if req.raw_mode:
            ...
            try:
                ...
                try:
                    f = parse_apdu_lenient(self._raw.get())
                except ValueError:
                    return
    ...
    def _load_active(self):
        ...
        self._le.set(req.le)
        self._label.set(req.label)
        self._raw.set(req.raw or assemble_apdu(req.cla, req.ins, req.p1, req.p2, req.lc, req.data, req.le))
    ...
    def _duplicate_request(self):
        """Clone the current request into a new tab without copying its response."""
        self._commit_active()
        current = self._requests[self._active]
        duplicate = _ForgeRequest(
            cla=current.cla,
            ins=current.ins,
            p1=current.p1,
            p2=current.p2,
            ...
            data=current.data,
            le=current.le,
            raw_mode=current.raw_mode,
            label=current.label,
            raw=current.raw,
        )
        self._add_request(duplicate)

    def _rename_request(self):
        """Prompt for a new label for the active Forge request tab."""
    ...
    def load_transaction(self, tx):
        ...
        req = _ForgeRequest(
            ...
            le="",
            label=tx.comment or describe_apdu_fields(
                tx.request_cla,
                tx.request_ins,
                tx.request_p1,
                tx.request_p2,
    ...
    def load_apdu(self, raw_hex, *, label=""):
        """Open a new Forge request from raw command APDU hex."""
        try:
        ...
        req = _ForgeRequest(
            **fields,
            raw_mode=True,
            raw=raw_hex.replace(" ", "").upper(),
            label=label or (describe_apdu_fields(**fields) if fields else "Raw APDU"),
        )
        self._commit_active()
        if len(self._requests) == 1 and self._is_pristine(self._requests[0]):
            self._requests[0] = req
    ...
    def _on_preset_selected(self, _event=None):
        ...
        if not self._label.get().strip():
            self._label.set(self._preset_var.get())
        if self._raw_mode.get():
    ...
    def _on_label_changed(self, *_args):
        if not self._requests:
            return
        self._requests[self._active].label = self._label.get().strip()
        self._render_tabstrip()

    def _on_request_changed(self, *_args):
        if hasattr(self, "_request_summary"):
            self._update_request_summary()

    def _request_tab_label(self, index):
        req = self._requests[index]
        suffix = req.label.strip()
        if not suffix:
            try:
                suffix = describe_apdu_fields(req.cla, req.ins, req.p1, req.p2, req.lc, req.data, req.le)
            except Exception:
                suffix = ""
        if suffix and suffix != "INS 00":
            return f"#{index + 1} {suffix[:18]}"
        return f"#{index + 1}"

    def _update_request_summary(self):
        try:
            if self._raw_mode.get():
                try:
                    parsed = parse_apdu(self._raw.get())
                except ValueError:
                    parsed = parse_apdu_lenient(self._raw.get())
                fields = (
                    parsed["cla"],
                    parsed["ins"],
                    parsed["p1"],
            ...
            if not any(fields):
                self._request_summary.set("No request loaded.")
                return
            summary = describe_apdu_fields(*fields)
            raw = assemble_apdu(*fields)
            self._request_summary.set(f"{summary} | {len(raw) // 2} command bytes")
        except Exception:
            self._request_summary.set("Request is not a valid short APDU yet.")

    def _copy_request(self):
        try:
            raw = assemble_apdu(*self._current_fields())
        except ValueError as e:
            messagebox.showerror("Invalid APDU", str(e))
            return
    ...
    def _copy_response(self):
        req = self._requests[self._active]
        if req.resp_sw1 is None or req.resp_sw2 is None:
            return
        self._copy_to_clipboard(f"{req.resp_data}{req.resp_sw1:02X}{req.resp_sw2:02X}".upper())

    def _send_to_fuzzer(self):
        if not hasattr(self.root, "security_pane"):
            return
        try:
            raw = assemble_apdu(*self._current_fields())
        except ValueError as e:
            messagebox.showerror("Invalid APDU", str(e))
            return
        label = self._label.get().strip() or describe_apdu_fields(*self._current_fields())
        self.root.security_pane.load_fuzz_seed(raw, label=label)
    ...
    def _on_sm_choice(self, _event=None):
        self._sm_user_forced_none = self._sm_var.get() == "None" and self._active_sm_label() is not None
    ...
    def _reestablish(self, mode):
        ...
        try:
            ...
            if doc_number and dob and expiry:
                ...
                if not build_mrz.check_mrz():
                    messagebox.showerror(
                        "Invalid MRZ",
                        "The Number / Date of Birth / Expiry at the top of the window do not form a valid MRZ.",
                    )
                    return
    ...
    def _get_ready(self):
        ...
        if not self.parent.reader:
            ...
            return False
        # Share the View tab's channel (and its live SM session); label any
        # traffic this tab drives that doesn't pass an explicit source.
        self.parent.ensure_iso7816().source = "forge"
        return True
    ...
    def _send(self):
        ...
        try:
            ...
            if not lc and data:
                lc = "%02x" % (len(data) // 2)
            # In fielded mode an empty LE means "request the maximum" (Le=00).
            # Raw mode preserves the exact byte string, including a deliberately
            # absent Le or a length-inconsistent APDU.
            if not le and not self._raw_mode.get():
                le = "00"
            ...
            req = self._requests[self._active]
            req.resp_data = to_hex_string(resp.data) if resp.data else ""
            ...
            sm_used = "none" if force_none or saved_ciphering is None else self._active_sm_label()
            logging.info(
                f"FORGE REQUEST: CLA:{cla} INS:{ins} P1:{p1} P2:{p2} LC:{lc} DATA:{data} LE:{le} SM:{sm_used}\n"
                f"FORGE RESPONSE: Data:{to_hex_string(resp.data) if resp.data else ''} "
