...
def _request_info(tx):
    ...
    header = " ".join(v.upper() for v in (tx.request_cla, tx.request_ins, tx.request_p1, tx.request_p2))
...
class TrafficPane:
    def __init__(self, main):
        ...
        self._pending_transactions: queue.Queue[tuple[int, APDUTransaction]] = queue.Queue()
        ...
        self._send_to_forge_button = ttk.Button(
            toolbar,
            text="Send to Forge",
            command=self._send_to_forge,
            state="disabled",
        )
        self._send_to_forge_button.pack(side="left", padx=3)
        self._send_to_fuzzer_button = ttk.Button(
            toolbar,
            text="Send to Fuzzer",
            command=self._send_to_fuzzer,
            state="disabled",
        )
        self._send_to_fuzzer_button.pack(side="left", padx=3)
        ...
        ttk.Button(filter_bar, text="✕", width=2, command=lambda: self._filter_text.set("")).pack(
            side="left", padx=(0, 12)
        )

        self._errors_only = tk.BooleanVar(value=False)
        ttk.Checkbutton(filter_bar, text="Errors only", variable=self._errors_only, command=self._rebuild_tree).pack(
            side="left", padx=4
        )
        self._hide_sm = tk.BooleanVar(value=False)
        ttk.Checkbutton(filter_bar, text="Hide SM", variable=self._hide_sm, command=self._rebuild_tree).pack(
            side="left", padx=4
        )
        ...
        self._source_combo = ttk.Combobox(
            ...
            width=12,
            values=[_SOURCE_ANY],
            postcommand=self._refresh_source_options,
        )
        ...
        self._wire_toggle = ttk.Checkbutton(
            legend,
            text="Show wire (encrypted) bytes",
            variable=self._show_wire,
            command=self._refresh_dump,
            state="disabled",
        )
        ...
        self._tree.tag_configure("resp_ok", background=theme.OK_BG)
        self._tree.tag_configure("resp_err", background=theme.ERR_BG)
        ...
        self._rebuild_tree()
        self.root.after(100, self._drain_pending_transactions)
    ...
    def reload(self):
        """Repaint the list from the global history (e.g. after a session load)."""
        self._rebuild_tree()

    def _on_new_transaction(self, tx):
        # This listener may run from a background passport operation. Queue the
        # update and let the Tk thread perform all widget calls.
        idx = len(self._history) - 1
        self._pending_transactions.put((idx, tx))

    def _drain_pending_transactions(self):
        needs_rebuild = False
        while True:
            try:
                idx, tx = self._pending_transactions.get_nowait()
            except queue.Empty:
                break

            try:
                current = self._history[idx]
            except IndexError:
                needs_rebuild = True
                continue

            if current is not tx:
                needs_rebuild = True
                continue

            if not self._tree.exists(f"{idx}-req"):
                self._append_transaction(idx, tx)
    ...
    def _on_select(self, _event=None):
        ...
        self._wire_toggle.configure(state="normal" if self._wire_available(tx, direction) else "disabled")
        self._refresh_dump()
    ...
    def _selected_request_index(self):
        """Return the selected request transaction index, or None if ambiguous."""

        selected = self._tree.selection()
        if len(selected) != 1:
            return None
        idx_str, direction = selected[0].rsplit("-", 1)
        if direction != "req":
            return None
        return int(idx_str)

    def _update_send_actions(self):
        state = "normal" if self._selected_request_index() is not None else "disabled"
        self._send_to_forge_button.configure(state=state)
        self._send_to_fuzzer_button.configure(state=state)
        if hasattr(self, "_menu"):
            self._menu.entryconfigure("Send to Forge", state=state)
            self._menu.entryconfigure("Send to Fuzzer", state=state)
    ...
    def _send_to_forge(self):
        idx = self._selected_request_index()
        if idx is None:
            return
    ...
    def _send_to_fuzzer(self):
        ...
        if idx is None:
            return
        tx = self._history[idx]
        if hasattr(self.root, "security_pane"):
            self.root.security_pane.load_fuzz_seed(_command_hex(tx), label=_request_info(tx))

    # ── Context menu ────────────────────────────────────────────────────────────
    def _build_context_menu(self):
        menu = tk.Menu(self._tree, tearoff=0)
        menu.add_command(label="Send to Forge", command=self._send_to_forge)
        menu.add_command(label="Send to Fuzzer", command=self._send_to_fuzzer)
        menu.add_separator()
        menu.add_command(label="Copy as hex", command=self._copy_as_hex)
        menu.add_command(label="Copy as Python (APDUCommand(...))", command=self._copy_as_python)
        menu.add_separator()
        ...
        for label, color in _HIGHLIGHTS:
            highlight.add_command(label=label, command=partial(self._set_color, color))
